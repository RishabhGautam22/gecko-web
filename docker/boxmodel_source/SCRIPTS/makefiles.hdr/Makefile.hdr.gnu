...
ifeq ($(MODE),DEVEL)
     FFLAGS = -I${my_netcdf_fortran_inc} -g -O0 -fbounds-check -fbacktrace -finit-integer=-100000 -ffpe-trap=zero,overflow,invalid -Waliasing -Wampersand -Wconversion -Wsurprising -Wintrinsics-std -Wintrinsic-shadow -Wline-truncation -Wunused-variable -Wuninitialized -fdefault-real-8 -mcmodel=small
else
     FFLAGS = -I${my_netcdf_fortran_inc} -fopenmp -fdefault-real-8 -mcmodel=small
endif
...
ifeq ($(NCCONFIG),none)
        ...
        NETCDFLIBS_F    =       $(shell $(NCCONFIG) --flibs | awk '{for (i=1;i<=NF;i++) if(substr($$i,1,2)=="-L" || substr($$i,1,2)=="-l") st=st" "$$i}END{print st}')
        NETCDFLIBS_C    =       $(shell $(my_netcdf_c_bin)/nc-config --libs | awk '{for (i=1;i<=NF;i++) if(substr($$i,1,2)=="-L" || substr($$i,1,2)=="-l") st=st" "$$i}END{print st}')
