...
!--------------------------------------------------------------------------
  ...
  IF (nfix/=0) THEN
    CALL resetc(y)  ;  CALL resetc(ynew)
  ENDIF

  CALL iterfirst(anrat,y,0.,soa_fg,nsat,idasat,numaou,idaou,nvoc,y,yp,yl)
...
28 CONTINUE
  ...
  DO I=1,NUMIT
    IF (nfix/=0) THEN
      CALL resetc(y)  ;  CALL resetc(ynew)
    ENDIF
    CALL iterfirst(anrat,wbdf2,dt,soa_fg,nsat,idasat,numaou,idaou,nvoc,ynew,yp,yl)
    NFCN=NFCN+1
  ENDDO
...
60 CONTINUE
  ...
  DO I=1,NUMIT
    IF (nfix/=0) THEN
      CALL resetc(y)  ;  CALL resetc(ynew)
    ENDIF
    CALL iterfirst(anrat,wbdf2,dtg,soa_fg,nsat,idasat,numaou,idaou,nvoc,ynew,yp,yl)
    NFCN=NFCN+1
  ENDDO
