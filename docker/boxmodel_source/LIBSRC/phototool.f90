...
! ------------------------
  filu=12 
  OPEN(filu,FILE='jfile.phot',FORM='formatted',STATUS='old',IOSTAT=ios)
  IF (ios /= 0) THEN
    mesg1="File 'jfile.phot' cannot be open"
...
! -----------------------------
    ...
    CASE('PHOT')
      ...
      IF (narg /= 2) THEN    ! species label nteta
        lostop=.TRUE.
        WRITE(lout,*) "--error--, in readjvalue while reading phot"
        WRITE(lout,*) "           # of arg not equal 3:"
        WRITE(lout,*) keyword,' ',TRIM(line) 
        CYCLE rdline
      ENDIF
      cpline=line(posi(1,1):)
      ...
      ELSE
        IF (isza /= nsza) THEN
          mesg1="number of tabulated sza are not identical in jfile.phot"
          CALL stoperr(progname,mesg1)
        ENDIF
        DO i=1,nsza
          IF (val1(i)/=xsza(i)) THEN
            WRITE(lout,*) "--error--, in readjvalue while reading jfile.phot"
    ...
    CASE DEFAULT
      WRITE(lout,*) "--error--,  while reading in jfile.phot", keyword
...
! stop if error
  IF (lostop) THEN
    mesg1="Miscellaneous errors found while reading jfile.phot"
...
! ti is the greenwich mean time (in hour) - use tz to change to local time.    
  ...
  IF (flag_ZA) sza=fixedZA   ! fixed value
